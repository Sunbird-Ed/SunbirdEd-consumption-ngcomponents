...
*`<sb-member-list [layoutConfig]="{size:'medium', isBold:false, isSelectable:true,` 
     `view:'horizontal'}" [memberList]="range(15)" [isMenu]="false"(cardClick)="memberCardClick``  ($event)" (menuClick)="menuClick($event)"></`sb-member-list>`*|
